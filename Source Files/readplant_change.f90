...
!!      initialize locals in loop
        ...
        if (eof < 0) exit
        read (plantdb2_num,*,iostat=eof) bioe, hvstc, blaic, frgrw1, laimx1,    & 
          frgrw2, laimx2, dlaic, chtmxc, rdmxc
        if (eof < 0) exit
        read (plantdb2_num,*,iostat=eof) topt, tbase, cnyldc, cpyldc, bn1, bn2,  &
          bn3, bp1c, bp2c, bp3c
        if (eof < 0) exit
        read (plantdb2_num,*,iostat=eof) wsyfc, usle_c, gsic, vpdfr, frgmax,     &
          wavpc, co2hi, bioehi, rsdcopl, alaimin
        if (eof < 0) exit
        read (plantdb2_num,777,iostat=eof) bioleaf, yrsmat, biomxtrees, extcoef, &
